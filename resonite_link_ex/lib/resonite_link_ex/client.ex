defmodule ResoniteLinkEx.Client do
  ...
  def handle_disconnect(reason, state) when is_map(state) do
    _ = handle_disconnect(state.client_pid, map_disconnect_reason(reason))
    {:ok, state}
  end
  ...
  ## Examples
      ResoniteLinkEx.Client.start_link(:bad, [])
      {:error, :invalid_request}
  """
  ...
  ## Examples
      ResoniteLinkEx.Client.client_pid(:bad)
      {:error, :invalid_request}
  """
  ...
  ## Examples
      ResoniteLinkEx.Client.send_json(:bad, %{})
      {:error, :invalid_request}
  """
  ...
  ## Examples
      match?({:ok, _json}, ResoniteLinkEx.Client.encode_outbound(%{"k" => "v"}))
      true
  """
  ...
  ## Examples
      ResoniteLinkEx.Client.decode_inbound(~s({"k":"v"}))
      {:ok, %{"k" => "v"}}
  """
  ...
  ## Examples
      ResoniteLinkEx.Client.map_disconnect_reason({:remote, 1000, "ok"})
      :close_frame
  """
  ...
  ## Examples
      ResoniteLinkEx.Client.build_url(host: "localhost", port: 12512, path: "")
      "ws://localhost:12512"
  """
  ...
  ## Examples
      ...
      match?({:ok, _}, ResoniteLinkEx.Client.handle_connect(:connected, state))
      true
  """
  def handle_connect(_conn, state) do
    _ = set_reconnecting(state.client_pid, false)
  ...
  ## Examples
      state = %{client_pid: self(), opts: []}
      match?({:reply, {:text, _}, _}, ResoniteLinkEx.Client.handle_cast({:send_text, "hello"}, state))
      true
  """
  @impl true
  def handle_cast(:send_initial_session_request, state) do
    {:ok, request} = Protocol.encode_request("requestSessionData", %{})
    _ = register_pending(state.client_pid, request["messageId"], self())
  ...
  ## Examples
      state = %{client_pid: self(), opts: []}
      ResoniteLinkEx.Client.handle_frame({:binary, <<1, 2>>}, state)
      {:ok, state}
  """
  def handle_frame({:text, message}, state) do
    case decode_inbound(message) do
      {:ok, payload} -> _ = receive_response(state.client_pid, payload)
      {:error, _reason} -> _ = receive_response(state.client_pid, %{})
    end
