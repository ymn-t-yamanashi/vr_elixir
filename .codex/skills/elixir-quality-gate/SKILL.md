...
description: Elixir変更時に、このPJの品質ゲート（format/compile/check.docs/credo/test --cover）を実行し、失敗原因を最小差分で解消する。Elixirコード編集、コミット前確認、カバレッジ確認時に使う。
---
...
2. ローカル環境で以下を順に実行する。
```bash
cd resonite_link_ex && mix local.hex --force && mix deps.get && mix format --check-formatted && mix compile --warnings-as-errors && mix check.docs && mix credo --strict && mix test --cover
```
